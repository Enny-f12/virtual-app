...
export default function Navbar() {
  ...
  const { border, muted, primary, primaryDeep, primaryForeground, surfaceHover, headerBg } = colors;
    ...
    >
      ...
      {navOpen && (
        <div className="md:hidden px-5 pb-5 flex flex-col gap-3" style={{ borderTop: `1px solid ${border}` }}>
          ...
          <button
            className="zapa-tap text-[14px] font-semibold px-4 py-2.5 rounded-full text-left"
            style={{ background: surfaceHover }}
          >
            Log in
          </button>
          <button
            className="zapa-tap text-[14px] font-bold px-5 py-2.5 rounded-full"
            style={{ background: `linear-gradient(135deg, ${primary}, ${primaryDeep})`, color: primaryForeground }}
          >
            Get Started
          </button>
